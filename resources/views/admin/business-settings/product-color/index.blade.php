        ...
        <div class="col-md-12">
            <div class="card card-box">
                <div class="card-head">
                    ...
                    <a href="{{ route('product-color.create') }}" id="addRow"
                       class="btn btn-primary pull-right mr-3 mt-1">
                        Add product color
                        ...
                        <table class="table table-hover order-column full-width" id="example4">
                            ...
                            @foreach($colors as $key => $color)
                                <tr class="">
                                    ...
                                    <td class="center">{{ $color->details }}</td>
                                    <td class="center">{{ $color->status }}</td>
                                    <td class="center" style="width: 20px!important;">
                                        <a href="{{ route('product-color.edit',$color->id) }}"
                                           class="btn btn-tbl-edit btn-xs pull-left">
                                            <i class="fa fa-pencil"></i>
                                        </a>
                                        <form action="{{ route('product-color.destroy',$color->id) }}" method="POST">
                                            @csrf
                                            @method('delete')
